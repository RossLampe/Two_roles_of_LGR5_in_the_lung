################################################################################
# Heatmaps from VST transformed data, KEGG pathways.
#
#-------------------------------------------------------------------------------
# Ross Lampe
...
################################################################################
# Read the DE results file and reduce it to only significantly expressed genes.
...
# #Remove rows whose counts are all zeros
# res_signif_AvsB <- res_signif_AvsB[rowSums(res_signif_AvsB[10:13])>0,]
...
signif_AvsB_counts <- res_signif_AvsB   
...
dds_vst_AvsB_signif$Gene <- NULL
#nrow(dds_vst_AvsB_signif_sorted)

pheatmap(dds_vst_AvsB_signif,  
         scale = "row",
         legend = TRUE, 
         main = 'KEGG WNT pathway, ssc04310; Pig Lung, PD111',
         show_rownames=TRUE,
         fontsize_row=8,
         fontsize_col=10,
        # cluster_rows=FALSE,
...
################################################################################
# Heatmap, KEGG_TGFbeta_pathyway_ssc04350
#   LGR5+ vs LGR5- 
#
################################################################################
# Get the markers
...
pheatmap(dds_vst_AvsB_signif,    #t(dds_vst_AvsB_signif_Top) to reverse rows and columns
         scale = "row",
         legend = TRUE, 
         main = 'KEGG TGF-β pathway, ssc04350; Pig Lung, PD111',
         show_rownames=TRUE,
         fontsize_row=8,
         fontsize_col=10,
         cluster_rows=TRUE,
